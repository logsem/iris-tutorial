...
(**
  ...
  expresion and state. [efs] is a list of expresions representing new
  threads spawned. Finally [κs] represents a list of something called
  observations. We won't use these, so κs will remain empty.

  This relation isn't very nice to work with by it self. To fix this
...
(**
  Now lets define steps in terms of configurations. To model the
...
(**
  ...
  value. Conversely a thread is not stuck if it is either values or
  can take a step.
*)
...
(**
  So a program won't get stuck if all posible future configurations
  aren't stuck.

  [∀ t σ' e', rtc step ([e], σ) (t, σ') → e' ∈ t → not_stuck e' σ']

  Likewise a program satisfies a postcondition if when the main thread
  terminates to a value, that value satifies that post condition.
  ...
  specifies whether adequacy inclues the first condition.

  The proof that weakest precondition implies adequacy is:
  [heap_adequacy Σ `{!heapGpreS Σ} s e σ φ :
    ∀ _ : heapGS Σ, inv_heap_inv -∗ WP e @ s; ⊤ {{ v, ⌜φ v⌝}} →
    adequate s e σ (λ v σ, φ v)
  ]

  To use this statement we have to instatiate actually Σ. It needs to
...
(** Finally we can combine the peaces to prove adequacy. *)
