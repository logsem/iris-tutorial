...
(*
  Let's use this knowledge to prove a specification for a larger program
*)
Lemma wp_prog_add_2 : ⊢ WP prog + #2 {{v, ⌜v = #5⌝}}.
Proof.
  iStartProof.
  (*
    The first part of this program is to evaluate `prog`. So we can seperate the program into 2 parts.
    First we evaluate `prog`, then we take the result and add 2 to it.
    To do this we can use `wp_bind`.
  *)
  wp_bind prog.
  (*
    Now we have the problem that our post condition doesn't match the one we proved.
    To fix this we can use monitonicity of WP.
  *)
  iApply wp_mono.
  2: { iApply wp_prog. }
  iIntros "%_ ->".
...
*)
Lemma wp_prog_2 (Φ : val → iProp Σ) : (∀ v, ⌜v = #3⌝ -∗ Φ v) -∗ WP prog {{v, Φ v}}.
Proof.
  iIntros "HΦ".
  rewrite /prog.
  wp_alloc l as "Hl".
...
(*
  Hoare triples are an extended version of WP that contains a precondition.
  They are defined as `∀ Φ, Pre -∗ (▷ ∀ r0 .. rn, Post -∗ Φ v) -∗ WP e {{v, Φ v}}`.
  This may seem like very long and complicated definition, so let's look at it's parts.

  Like before, Hoare triples are parameterised on the post conditions that are satisfied by Post. This allows us to further specialize the posible return values by specifying it as a pattern quantified over arbitrary parameters.
  The implication of the post condition is hidden under a later, signifying that the program takes at least one step.
  And finaly we now have a precondition.

  The syntax for Hoare triples is as follows:
  `{{{ Pre }}} e {{{ r0 .. rn, RET v; Post }}}`
  - `Pre`: the precondiction that is assumed to hold before the program runs.
...
Lemma wp_swap (l1 l2 : loc) (v1 v2 : val) : {{{l1 ↦ v1 ∗ l2 ↦ v2}}} swap #l1 #l2 {{{RET #(); l1 ↦ v2 ∗ l2 ↦ v1}}}.
Proof.
  iIntros "%Φ [H1 H2] HΦ".
  rewrite /swap.
  wp_pures.
